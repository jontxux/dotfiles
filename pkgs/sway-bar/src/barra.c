    ...
    while (written < count) {
        ...
        if (w < 0) {
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                /* Socket no bloqueante: esperar con poll */
...
static void truncar_texto_utf8(char *destino, size_t tam_destino, const char *texto_original) {
    ...
    if (wchar_count == (size_t)(-1)) {
        ...
        snprintf(destino, tam_destino, "%s", texto_original);
        return;
    }
...
static void cb_info_servidor(pa_context *c, const pa_server_info *info, void *userdata) {
    ...
    if (info && info->default_sink_name) {
        snprintf(cx->estado.nombre_sumidero, sizeof(cx->estado.nombre_sumidero), "%s", info->default_sink_name);
    }
    pthread_mutex_unlock(&cx->candado);
...
static void cb_estado_pa(pa_context *c, void *userdata) {
    ...
    if (st == PA_CONTEXT_READY) {
        ...
        /* PulseAudio se desconectó - intentar reconectar */
        fprintf(stderr, "PulseAudio desconectado (estado %d), intentando reconectar...\n", st);
        
        pthread_mutex_lock(&cx->candado);
        cx->estado.listo = false;
        pthread_mutex_unlock(&cx->candado);

        /* Desconectar contexto antiguo */
        ...
        if (!cx->ctx) {
            fprintf(stderr, "Error: no se pudo recrear pa_context\n");
            return;
        }
...
static int conectar_sway_ipc(const char *socket_path) {
    ...
    /* Configurar socket como no-bloqueante para evitar bloqueos si sway no responde */
    ...
    if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        ...
        /* connect en progreso - esperar con poll */
        struct pollfd pfd = {.fd = fd, .events = POLLOUT};
        if (poll(&pfd, 1, 5000) < 0 || !(pfd.revents & POLLOUT)) {
            close(fd);
            return -1;
    ...
    for (int i = 0; i < len && !*nombre_salida; i++) {
        ...
        if (json_object_object_get_ex(item, "focused", &focused) &&
            ...
            if (tipo_nodo && (strcmp(tipo_nodo, "con") == 0 || strcmp(tipo_nodo, "floating_con") == 0)) {
                ...
                const char *nombre_ventana = NULL;

                /* Primero intentar obtener el nombre/título de la ventana */
                if (json_object_object_get_ex(item, "name", &name_obj) &&
                        json_object_is_type(name_obj, json_type_string)) {
                    const char *nombre = json_object_get_string(name_obj);
                    if (nombre && strlen(nombre) > 0) {
                        nombre_ventana = nombre;
                    }
                }

                /* Si no hay título de ventana, usar app_id o pid (estrategia yambar) */
                if (!nombre_ventana) {
                    struct json_object *app_id_obj = NULL;
                    struct json_object *pid_obj = NULL;
                    
                    /* Intentar obtener app_id primero */
                    if (json_object_object_get_ex(item, "app_id", &app_id_obj) &&
                        ...
                        if (app_id && strlen(app_id) > 0) {
                            /* Si también hay PID, intentar leer /proc/<pid>/comm para nombre más preciso */
                            if (json_object_object_get_ex(item, "pid", &pid_obj) &&
                                    json_object_is_type(pid_obj, json_type_int)) {
                                int pid = (int)json_object_get_int(pid_obj);
                                if (pid > 0) {
                                    char ruta_proc[64];
                                    snprintf(ruta_proc, sizeof(ruta_proc), "/proc/%d/comm", pid);
                                    int fd = open(ruta_proc, O_RDONLY | O_CLOEXEC);
                                    if (fd >= 0) {
                                        static char comm_buffer[128];
                                        ssize_t bytes = read(fd, comm_buffer, sizeof(comm_buffer) - 1);
                                        close(fd);
                                        if (bytes > 0) {
                                            comm_buffer[bytes] = '\0';
                                            /* Eliminar newline */
                                            size_t comm_len = strlen(comm_buffer);
                                            if (comm_len > 0 && comm_buffer[comm_len - 1] == '\n') {
                                                comm_buffer[comm_len - 1] = '\0';
                                            }
                                            if (strlen(comm_buffer) > 0) {
                                                nombre_ventana = strdup(comm_buffer);
                                            }
                                        }
                                    }
                                }
                            }
                            /* Si no se pudo obtener de /proc, usar app_id directamente */
                            if (!nombre_ventana) {
                                nombre_ventana = app_id;
                            }
                        }
                    }
                }

                /* Si aún no hay nombre, intentar con class (X11) */
                if (!nombre_ventana) {
                    struct json_object *window_props = NULL;
                    if (json_object_object_get_ex(item, "window_properties", &window_props) &&
                            json_object_is_type(window_props, json_type_object)) {
                        struct json_object *class_obj = NULL;
                        if (json_object_object_get_ex(window_props, "class", &class_obj) &&
                                json_object_is_type(class_obj, json_type_string)) {
                            const char *clase = json_object_get_string(class_obj);
                            if (clase && strlen(clase) > 0) {
                                nombre_ventana = clase;
                            }
                        }
                    }
                }

                /* Si hay un nombre válido, usarlo */
...
static void actualizar_foco_desde_sway(struct Contexto *cx, int sway_fd) {
    ...
    if (rtype == GET_TREE && reply) {
        ...
        pthread_mutex_lock(&cx->candado);
        snprintf(cx->estado.foco_actual, sizeof(cx->estado.foco_actual), "%s", nombre_truncado);
        pthread_mutex_unlock(&cx->candado);
...
/* Extraer nombre de ventana directamente del mensaje de evento (más rápido que GET_TREE) */
static char *extraer_foco_de_evento(const char *json_str, uint32_t len) {
    if (!validar_json_entrada(json_str, len)) {
        return NULL;
    }
    ...
    /* Verificar que es evento de foco */
    struct json_object *change_obj = NULL;
    if (!json_object_object_get_ex(root, "change", &change_obj) ||
            !json_object_is_type(change_obj, json_type_string)) {
        json_object_put(root);
        return NULL;
    }

    const char *change = json_object_get_string(change_obj);
    if (strcmp(change, "focus") != 0) {
        json_object_put(root);
        return NULL;
    }

    /* Extraer container del evento */
    struct json_object *container = NULL;
    if (!json_object_object_get_ex(root, "container", &container)) {
        json_object_put(root);
        return NULL;
    }

    /* Obtener nombre de la ventana */
    struct json_object *name_obj = NULL;
    const char *nombre = NULL;
    if (json_object_object_get_ex(container, "name", &name_obj) &&
            json_object_is_type(name_obj, json_type_string)) {
        nombre = json_object_get_string(name_obj);
    }

    /* Si no hay nombre, intentar con app_id + pid (estrategia yambar) */
    if (!nombre || strlen(nombre) == 0) {
        struct json_object *app_id_obj = NULL;
        struct json_object *pid_obj = NULL;

        if (json_object_object_get_ex(container, "app_id", &app_id_obj) &&
                json_object_is_type(app_id_obj, json_type_string)) {
            const char *app_id = json_object_get_string(app_id_obj);

            /* Intentar obtener nombre real de /proc/<pid>/comm */
            if (json_object_object_get_ex(container, "pid", &pid_obj) &&
                    json_object_is_type(pid_obj, json_type_int)) {
                int pid = (int)json_object_get_int(pid_obj);
                if (pid > 0) {
                    char ruta_proc[64];
            ...
            /* Si no se pudo obtener de /proc, usar app_id */
            if (!nombre && app_id && strlen(app_id) > 0) {
                nombre = app_id;
            }
        }
    }
...
static void crear_bloque_foco(struct json_object *array, const struct EstadoAudio *estado) {
    ...
    json_object_object_add(bloque, "min_width", json_object_new_int(1280));
...
static void crear_bloque_volumen(struct json_object *array, const struct EstadoAudio *estado) {
    ...
    json_object_object_add(bloque, "min_width", json_object_new_int(100));
    json_object_object_add(bloque, "align", json_object_new_string("center"));
    json_object_array_add(array, bloque);
}
...
static void crear_bloque_fecha(struct json_object *array) {
    ...
    json_object_object_add(bloque, "separator", json_object_new_boolean(false));
    json_object_object_add(bloque, "min_width", json_object_new_int(200));
    json_object_object_add(bloque, "align", json_object_new_string("center"));
    json_object_array_add(array, bloque);
}
...
static void manejar_evento_sway(int sway_fd, struct Contexto *cx, int *nfds) {
    ...
    if (rc == 1) {
        /* EOF: socket cerrado por sway - intentar reconexión */
    ...
    /* Intentar extraer foco directamente del mensaje del evento */
    char *nombre_foco = NULL;
    if (ev_msg && ev_type == 0) {  /* tipo 0 = evento window */
        nombre_foco = extraer_foco_de_evento(ev_msg, ev_len);
    }

    /* Liberar mensaje del evento */
    if (ev_msg) free(ev_msg);

    /* Si se extrajo el foco del evento, actualizar estado directamente */
    if (nombre_foco) {
        char nombre_truncado[TAM_TEXTO_TRUNCADO];
        truncar_texto_utf8(nombre_truncado, sizeof(nombre_truncado), nombre_foco);
        ...
        /* Fallback: consultar árbol completo si el evento no tenía información */
...
static void ejecutar_bucle_principal(struct Contexto *cx, int tfd, int sway_fd) {
    ...
    time_t ultimo_intento_reconexion = 0;
    ...
    for (;;) {
        ...
        /* Intentar reconexión a sway si se perdió la conexión */
        if (sway_fd < 0 && nfds == 2) {
            time_t ahora = time(NULL);
            if (ahora - ultimo_intento_reconexion >= 5) {
                fprintf(stderr, "Intentando reconectar a sway...\n");
                int nuevo_sway_fd = configurar_sway_ipc();
                if (nuevo_sway_fd >= 0) {
                    sway_fd = nuevo_sway_fd;
                    nfds = 3;
                    fds[2].fd = sway_fd;
                    fds[2].events = POLLIN;
                    fprintf(stderr, "Reconexión exitosa\n");
                    /* Actualizar foco inicial tras reconectar */
                    actualizar_foco_desde_sway(cx, sway_fd);
                }
                ultimo_intento_reconexion = ahora;
            }
        }
    }
}
